...
- `mobius_template.pov` - Main scene file; clock-driven Möbius motion, no photons
- `math.inc` - Inverse-stereographic `SU`/`SV` projection and grid `Line` helper
- `setup.inc` - Palette, patch/wire masks, material tuning
- `textures.inc` - Floor, glass, rainbow patch, wire and glow textures
